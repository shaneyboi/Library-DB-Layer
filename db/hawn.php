...
class Hawn
{
     ...
     **/
	public $table_name = 'view_hawn_ic_all';
    ...
    public function change_tbl($table){
        switch ($table) {
            ...
            case 'view':
                $tbl[0] = "view_hawn_ic_all";
                $tbl[1] = "fld_id";
                break;

            case 'table':
                $tbl[0] = "view_hawn_stats";
                $tbl[1] = "the_date";
                break;

            case 'tally':
                $tbl[0] = "tbl_hawn_item_nobarcode INNER JOIN tbl_location_stat ON fld_stat_id=fld_location_num";
                $tbl[1] = "fld_recorded";
                break;

            case 'scan':
            default:
                $tbl[0] = "tbl_hawn_item_scanned";
                $tbl[1] = "fld_id";
                break;
        }
    ...
    /**
     *  Function that gets the result from id.
     *  @param - datetime - $id - the datetime used to insert the scanened item.. 
     *  @return array | false - returns an array of array which will only return the 1st row of the array. 
     * */
    public function get_via_timestamp($id){
        $this->change_tbl('view');
        $rs = $this->select("fld_datetime = :fld_datetime",'fld_id DESC' , array("fld_datetime" => $id));
        if($rs && count($rs)){
            return $rs[0];
        }
        return false;
    ...
    public function insert($record = array()){
        $query="INSERT INTO tbl_hawn_item_scanned (fld_date_scan, fld_item_scan, fld_location_code) VALUES (:current_datetime, :item, :location)";
        return $this->run($query, $record);
    }
    ...
    public function tally($record = array()){
        $query = "INSERT INTO tbl_hawn_item_nobarcode (fld_recorded, fld_stat_id) VALUE (:current_datetime, :locationId)";
        return $this->run($query, $record);
    }
